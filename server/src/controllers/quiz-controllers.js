...
async function fetchEntry(schema, id, requiredProperties = []) {
    ...
    for(const property of requiredProperties) {
        if(!entry[property]) {
            throw new Error(`${schema.modelName} with id: ${id} has no property ${property}`);
        }
    }
...
exports.createQuiz = async(req, res, next) => {
    try {
        ...
        const quiz = await Quiz.findOneAndUpdate({owner:userId},{flashcards:flashcards,numRemainingFlashcards:flashcards.length},{new:true, upsert:true})
